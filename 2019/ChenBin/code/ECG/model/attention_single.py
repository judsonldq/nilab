...
def run_model(write_path, data, para_dict):
    ...
    third_block = conv_block(input_name_list=second_block, input_shape=single_length, ratio=ratio,
...
def attention_module(input_name, ratio, base):
    ...
    dense_output_list[0] = Dense(int(input_name.shape[-1]), activation='sigmoid')
...
def attention_experiment(para_dict):
    ...
    acc_file_path = './fixed_length.txt'
    acc_list = []
