...
def run_model(write_path, data, para_dict):
    ...
    q_length = data['x_train']['qrs_wave'].shape[1:]
    ...
    first_block_single = conv_block(input_name_list=[input_beat], input_shape=single_length, ratio=ratio,
                                    name='beat', filter_factor=1, bias=bias, attention=False, mode='input')
    # second_block_single = conv_block(input_name_list=first_block_single, input_shape=single_length, ratio=ratio,
    #                                  name='beat', filter_factor=2, bias=bias, attention=False)
    third_block_single = conv_block(input_name_list=first_block_single, input_shape=single_length, ratio=ratio,
                                    name='beat', filter_factor=4, bias=bias, mode='output')
    # p wave
    input_p_wave = Input(shape=p_length)
    first_block_p = conv_block(input_name_list=[input_p_wave], input_shape=p_length, ratio=ratio,
                               name='p', filter_factor=1, bias=bias, attention=False, mode='input')
    # second_block_p = conv_block(input_name_list=first_block_p, input_shape=p_length, ratio=ratio,
    #                             name='p', filter_factor=2, bias=bias, attention=False)
    third_block_p = conv_block(input_name_list=first_block_p, input_shape=p_length, ratio=ratio,
                               name='p', filter_factor=4, bias=bias, mode='output')
    # q wave
    input_q_wave = Input(shape=q_length)
    first_block_q = conv_block(input_name_list=[input_q_wave], input_shape=q_length, ratio=ratio,
                               name='q', filter_factor=1, bias=bias, attention=False, mode='input')
    # second_block_q = conv_block(input_name_list=first_block_q, input_shape=q_length, ratio=ratio,
    #                             name='q', filter_factor=2, bias=bias, attention=False)
    third_block_q = conv_block(input_name_list=first_block_q, input_shape=q_length, ratio=ratio,
                               name='q', filter_factor=4, bias=bias, mode='output')
    # t wave
    input_t_wave = Input(shape=t_length)
    first_block_t = conv_block(input_name_list=[input_t_wave], input_shape=t_length, ratio=ratio,
                               name='t', filter_factor=1, bias=bias, attention=False, mode='input')
    # second_block_t = conv_block(input_name_list=first_block_t, input_shape=t_length, ratio=ratio,
    #                             name='t', filter_factor=2, bias=bias, attention=False)
    third_block_t = conv_block(input_name_list=first_block_t, input_shape=t_length, ratio=ratio,
                               name='t', filter_factor=4, bias=bias, mode='output')
    # tri beat
    input_tri= Input(shape=tri_length)
    first_block_tri = conv_block(input_name_list=[input_tri], input_shape=tri_length, ratio=ratio,
                                 name='tri', filter_factor=1, bias=bias, attention=False, mode='input')
    # second_block_tri = conv_block(input_name_list=first_block_tri, input_shape=tri_length, ratio=ratio,
    #                                  name='tri', filter_factor=2, bias=bias, attention=False)
    third_block_tri = conv_block(input_name_list=first_block_tri, input_shape=tri_length, ratio=ratio,
                                    name='tri', filter_factor=4, bias=bias, mode='output')
    # hand feature
    hand_input = Input(shape=hand_length)
    dense_hand_1 = Dense(8, activation='relu')(hand_input)
    dropout_hand_1 = Dropout(0.1)(dense_hand_1)
    ...
    all_input = concatenate([third_block_single[0], third_block_p[0], third_block_q[0], third_block_t[0],
                             third_block_tri[0], hand_output])
    # 全连接层
    ...
    model = Model(inputs=[input_beat, input_p_wave, input_q_wave, input_t_wave, input_tri, hand_input], outputs=output)
    ...
    history = model.fit(x=[data['x_train']['single_beat'], data['x_train']['p_wave'], data['x_train']['qrs_wave'],
                           data['x_train']['t_wave'], data['x_train']['tri_beat'], data['x_train']['hand_feature']],
    ...
    predict_classes = model.predict([data['x_test']['single_beat'], data['x_test']['p_wave'], data['x_test']['qrs_wave'],
                                     data['x_test']['t_wave'], data['x_test']['tri_beat'],
                                     data['x_test']['hand_feature']], batch_size=32, verbose=2)
...
def attention_module(input_name, ratio, base):
    ...
    repeat_list[0] = RepeatVector(int(input_name.shape[1]))(add_list[0])
