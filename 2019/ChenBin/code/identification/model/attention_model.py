...
def build_model(data, para_dict):
    ...
    #     input_shape_list.append(np.shape(data['x_train'][i])[1:])
    ...
    dense_2 = Dense(512, kernel_initializer='glorot_uniform',
                    bias_initializer='glorot_uniform')(all_input)
    activation_2 = LeakyReLU(alpha=0.05)(dense_2)
    dropout_2 = Dropout(0.2)(activation_2)
    bn_2 = BatchNormalization()(dropout_2)
    dense_3 = Dense(128, kernel_initializer='glorot_uniform',
                    bias_initializer='glorot_uniform')(bn_2)
    activation_3 = LeakyReLU(alpha=0.05)(dense_3)
    dropout_3 = Dropout(0.2)(activation_3)
...
def run_model(model, data, i, log_path, write_path=""):
    """
    模型训练
    ...
    tensorboad = TensorBoard(log_dir=log_path)
    checkpoint = ModelCheckpoint(filepath=log_path+str(i)+'.h5',
    ...
    history = model.fit(x=train_x, y=data['y_train'], batch_size=32, epochs=100,
...
def model_flow(read_path, log_path, write_path, para_dict):
    # acc_list = []
    lead_list = list(np.arange(12))
    data = out_data(read_path=read_path, lead_list=lead_list, level=5)
    write_file = write_path + "150_subjects.txt"
    for times in range(3):
        model = build_model(data, para_dict)
        run_model(model, data, times, log_path, write_file)
